defmodule Cuerdo.ArazzoCase do
  ...
  defmacro arazzo_document_test(opts \\ []) do
    ...
    valid_ids = Enum.map(document.workflows, & &1.workflowId)

    workflow_ids =
      evaluated_opts
      |> Keyword.get(:only, valid_ids)
      |> Enum.reject(&(&1 in Keyword.fetch!(evaluated_opts, :exclude)))

    case Enum.filter(workflow_ids, &(not Enum.member?(valid_ids, &1))) do
      [] -> nil
      ids -> raise ArgumentError, "invalid workflow ids: #{Enum.join(ids, ", ")}"
    end
